<?php
	...
	class Plataforma
	{
		...
		public static function loggout(){
			...
			session_destroy();
			header('Location: '.INCLUDE_PATH_PLATAFORMA);
		}
		...
		public static function carregarCategoria($id){

			$sql = MySql::conectar()->prepare("SELECT DISTINCT tb_categoria.nome_categoria
										 FROM tb_categoria
		...
		public static function uploadFile($file){
			...
			if(move_uploaded_file($file['tmp_name'],BASE_DIR_PLATAFORMA.'/uploads/'.$imagemNome))
				return $imagemNome;
			else
				return false;
		}
		...
		public static function selecionaTudo($tabela){
		...
		*/
		...
		public static function orderItem($tabela,$orderType,$idItem){
			if($orderType == 'up'){
				$infoItemAtual = Plataforma::select($tabela,'id=?',array($idItem));
				...
				Plataforma::update(array('nome_tabela'=>$tabela,'id'=>$itemBefore['id'],'order_id'=>$infoItemAtual['order_id']));
				Plataforma::update(array('nome_tabela'=>$tabela,'id'=>$infoItemAtual['id'],'order_id'=>$itemBefore['order_id']));
